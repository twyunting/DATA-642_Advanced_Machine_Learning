...
Because $A$ is positive definite so the left hand side is positive, and $V$ is an non-zero eigenvector. Also, the length $||V||^2$ must be positive, we can derive the eigenvalues of a symmetric positive matrix $\lambda$ is positive. It follows that every eigenvalue $\lambda$ of $A$ is real.
$$
